...
class DAC_ADCServer(DeviceServer):
    ...
    @setting(3737, channelOut='i', channelIn='i', setpoint='v', kp='v', ki='v', kd='v', iterations='i', threshold='v', timeconstant='v', returns='b')
    def setfeedback(self, c, channelOut, channelIn, setpoint, kp, ki, kd, iterations, threshold, timeconstant):
        dev = self.selectedDevice(c)        
        yield dev.write("SETFB,%i,%i,%s,%s,%s,%s,%i,%s,%s\r"%(channelOut, channelIn, setpoint, kp, ki, kd, iterations, threshold, timeconstant))
        returnValue(True)

    @setting(3738, onoff='b', returns='b')
    def feedbackOnOff(self, c, onoff):
        dev = self.selectedDevice(c)
        yield dev.write("FBSWITCH,%i\r"%onoff)
        returnValue(True)

    @setting(3939,dacPorts='*i', adcPorts=['*i','i'], ivoltages='*v[]', fvoltages='*v[]', steps='i',delay='v[]',nReadings='i',returns='**v[]')#(*v[],*v[])')
    def buffer_feedback(self,c,dacPorts,adcPorts,ivoltages,fvoltages,steps,delay,nReadings=1):
        """
        BUFFER_RAMP ramps the specified output channels from the initial voltages to the final voltages and reads the specified input channels in a synchronized manner. 
        It does it within an specified number steps and a delay (microseconds) between the update of the last output channel and the reading of the first input channel.
        """
        ...
        dev = self.selectedDevice(c)
        yield dev.write("BUFFB,%s,%s,%s,%s,%i,%i,%i\r" % (sdacPorts, sadcPorts, sivoltages, sfvoltages, steps, delay, nReadings))
        self.sigBufferRampStarted([dacPorts, adcPorts, ivoltages, fvoltages, str(steps), str(delay), str(nReadings)])
